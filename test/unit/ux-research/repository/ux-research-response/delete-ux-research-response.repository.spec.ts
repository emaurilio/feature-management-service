/* eslint-disable @typescript-eslint/no-unsafe-argument */
/* eslint-disable @typescript-eslint/unbound-method */
import { Test, TestingModule } from '@nestjs/testing';
import { DataSource } from 'typeorm';
import { UXResearchResponseRepository } from 'src/ux-research/infraestructure/persistence/repositories/ux-research-response.repository';

describe('UXResearchResponseRepository - deleteUXResearchResponse', () => {
  let repository: UXResearchResponseRepository;

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        UXResearchResponseRepository,
        {
          provide: DataSource,
          useValue: {
            createEntityManager: jest.fn().mockReturnValue({}),
          },
        },
      ],
    }).compile();

    repository = module.get<UXResearchResponseRepository>(
      UXResearchResponseRepository,
    );
  });

  it('should be defined', () => {
    expect(repository).toBeDefined();
  });

  it('should delete UX research response successfully', async () => {
    const id = 'ux-research-response-1';
    const mockSoftDeleteResult = { affected: 1, generatedMaps: [], raw: {} };

    jest.spyOn(repository, 'softDelete').mockResolvedValue(mockSoftDeleteResult);

    const result = await repository.deleteUXResearchResponse(id);

    expect(repository.softDelete).toHaveBeenCalledWith(id);
    expect(result).toBe(true);
  });

  it('should return false when no record is deleted', async () => {
    const id = 'non-existent-id';
    const mockSoftDeleteResult = { affected: 0, generatedMaps: [], raw: {} };

    jest.spyOn(repository, 'softDelete').mockResolvedValue(mockSoftDeleteResult);

    const result = await repository.deleteUXResearchResponse(id);

    expect(repository.softDelete).toHaveBeenCalledWith(id);
    expect(result).toBe(false);
  });

  it('should handle database errors gracefully', async () => {
    const id = 'ux-research-response-1';
    const error = new Error('Database connection failed');

    jest.spyOn(repository, 'softDelete').mockRejectedValue(error);

    await expect(repository.deleteUXResearchResponse(id))
      .rejects.toThrow('Database connection failed');

    expect(repository.softDelete).toHaveBeenCalledWith(id);
  });

  it('should work with different ID formats', async () => {
    const id = 'uuid-format-123e4567-e89b-12d3-a456-426614174000';
    const mockSoftDeleteResult = { affected: 1, generatedMaps: [], raw: {} };

    jest.spyOn(repository, 'softDelete').mockResolvedValue(mockSoftDeleteResult);

    const result = await repository.deleteUXResearchResponse(id);

    expect(repository.softDelete).toHaveBeenCalledWith(id);
    expect(result).toBe(true);
  });

  it('should handle softDelete returning undefined affected', async () => {
    const id = 'ux-research-response-1';
    const mockSoftDeleteResult = { affected: undefined, generatedMaps: [], raw: {} };

    jest.spyOn(repository, 'softDelete').mockResolvedValue(mockSoftDeleteResult);

    const result = await repository.deleteUXResearchResponse(id);

    expect(repository.softDelete).toHaveBeenCalledWith(id);
    expect(result).toBe(false);
  });

  it('should handle softDelete returning null affected', async () => {
    const id = 'ux-research-response-1';
    const mockSoftDeleteResult = { affected: undefined, generatedMaps: [], raw: {} };

    jest.spyOn(repository, 'softDelete').mockResolvedValue(mockSoftDeleteResult);

    const result = await repository.deleteUXResearchResponse(id);

    expect(repository.softDelete).toHaveBeenCalledWith(id);
    expect(result).toBe(false);
  });

  it('should handle multiple records being deleted', async () => {
    const id = 'ux-research-response-1';
    const mockSoftDeleteResult = { affected: 3, generatedMaps: [], raw: {} };

    jest.spyOn(repository, 'softDelete').mockResolvedValue(mockSoftDeleteResult);

    const result = await repository.deleteUXResearchResponse(id);

    expect(repository.softDelete).toHaveBeenCalledWith(id);
    expect(result).toBe(true);
  });
});