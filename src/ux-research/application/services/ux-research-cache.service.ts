...
@Injectable()
export class UXResearchCacheService {
  private readonly logger = new Logger(UXResearchCacheService.name);
    ...
    try {
      await Promise.all(
        entitiesId.map((entityId) =>
          this.cacheManager.del(`${entityId}-${researchName}-${version}`),
        ),
      );
    } catch (error) {
      const errorMessage = getErrorMessage(error);
      this.logger.error(
        `Failed to invalidate cache for key ${researchName} v${version}`,
        errorMessage,
      );
